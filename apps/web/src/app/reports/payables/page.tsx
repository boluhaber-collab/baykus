...
export default function PayablesReportPage() {
  ...
  const rows = useMemo(() => {
    ...
    if (needle) {
      list = list.filter((r) =>
        [r.code, r.name, r.city, r.phone].join(" ").toLocaleLowerCase("tr").includes(needle),
      );
    }
  ...
  return (
    <div>
      ...
      <SummaryCards
        items={[
          { label: "Tedarikçi", value: rows.length || Number(s.supplier_count ?? 0) },
          ...
          { label: "Filtreli", value: formatMoney(filteredTotal), accent: "border-sky-200" },
        ]}
      />
